...
internal struct PointMarkers<T>: ViewModifier where T: CTLineChartDataProtocol & GetDataProtocol {
    ...
    internal init(chartData: T, startIndex: Int) {
    ...
    internal func body(content: Content) -> some View {
        ZStack {
            if chartData.isGreaterThanTwo() {
                content
                chartData.getPointMarker(startIndex: startIndex)
            } else { content }
        }
    }
}
...
extension View {
     ...
     */
    public func pointMarkers<T: CTLineChartDataProtocol & GetDataProtocol>(chartData: T, startIndex: Int) -> some View {
        self.modifier(PointMarkers(chartData: chartData, startIndex: startIndex))
    }
}
