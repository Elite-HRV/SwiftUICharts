...
public final class LineChartData: CTLineChartDataProtocol, GetDataProtocol, Publishable, PointOfInterestProtocol {
                        ...
                        GeometryReader { geo in
                            if let label = self.dataSets.dataPoints[i].xAxisLabel {
                                if label != "" {
                                    ...
                                    // 0.5 and 14 are offsets
                                        .frame(width: ((geo.frame(in: .local).width * 0.5) / CGFloat(self.dataSets.dataPoints.count - 1)),
                                               height: self.viewData.xAxisLabelHeights.max() ?? 0)
                                        .offset(x: CGFloat(i) * ((geo.frame(in: .local).width * 0.95) / CGFloat(self.dataSets.dataPoints.count - 1)),
                                                y: 0)
                                }
                            }
                        }
    ...
    // MARK: Points
    public final func getPointMarker(startIndex: Int) -> some View {
